...
import '../../video/pages/video_feed_page.dart';
import '../../video/core/models/feed_video_item.dart';
...
class _ProfileVideosTabState extends ConsumerState<ProfileVideosTab> {
  ...
  Future<void> _loadFolders() async {
    ...
    res.fold(
      ...
      (folders) {
        print(
          "🎬 [VideosTab] Found ${folders.length} video folders in DB: $folders",
        );
        if (mounted) {
          setState(() {
            _folders = folders;
  ...
  Widget build(BuildContext context) {
    print(
      "🏗️🏗️🏗️ [VideosTab] build CALLED. Folders loaded: $_foldersLoaded, Count: ${_folders.length}",
    );
                  ...
                  PagedSliverMasonryGrid.count(
                    ...
                    builderDelegate: PagedChildBuilderDelegate<CharterModel>(
                      itemBuilder: (context, item, index) => MediaGridItem(
                        post: item,
                        onTap: () {
                          final List<FeedVideoItem> feedItems = state.pages!
                              .expand((page) => page)
                              .map(
                                (item) => FeedVideoItem(
                                  id: item.id,
                                  videoUrl: item.videoUrl ?? '',
                                  title: item.currentChartName ?? 'Post',
                                  description: item.title,
                                  authorName: item.username,
                                  authorAvatarUrl: item.profileImageUrl,
                                  isChannelPost: false,
                                ),
                              )
                              .toList();
                          ...
                          Navigator.push(
                            ...
                            MaterialPageRoute(
                              builder: (context) => VideoFeedPage(
                                videos: feedItems,
                                initialIndex: index,
                                initialTab: VideoFeedTab.friends,
                              ),
                            ),
                          );
                        },
