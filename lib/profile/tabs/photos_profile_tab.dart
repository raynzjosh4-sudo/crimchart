...
class ProfilePhotosTab extends ConsumerStatefulWidget {
  ...
  const ProfilePhotosTab({super.key, this.userId});
...
class _ProfilePhotosTabState extends ConsumerState<ProfilePhotosTab> {
  ...
  late final PagingController<int, CharterModel> _pagingController =
    ...
    fetchPage: (pageKey) async {
      ...
      final res = await repo.getUserPosts(
        targetUserId,
        isVideo: false,
        isAudio: false,  // 👑 Exclude audio posts — they caused ImageDecoder crashes
      ...
      return res.fold(
        (fail) {
          print("📸 [PhotosTab] DB Fetch Failed: ${fail.message}");
          throw fail.message;
        },
        (posts) {
  ...
  Future<void> _loadFolders() async {
    final targetUserId = widget.userId ?? ref.read(authControllerProvider).user?.id;
    print("📸 [PhotosTab] Loading folders for user: $targetUserId");
    
    if (targetUserId == null) return;

    final repo = ref.read(feedRepositoryProvider);
    final res = await repo.getUserPostFolders(
      targetUserId,
      isVideo: false,
      isAudio: false,  // 👑 Only photo folders
    );

    res.fold(
      (fail) => print("📸 [PhotosTab] Folder Fetch Error: ${fail.message}"),
      (folders) {
        print("📸 [PhotosTab] Found ${folders.length} folders in DB: $folders");
        if (mounted) {
          setState(() {
            _folders = folders;
            _foldersLoaded = true;
  ...
  void _onFolderSelected(String? folder) {
    print("📸 [PhotosTab] Folder selected: $folder");
  ...
  Widget build(BuildContext context) {
    print("🏗️🏗️🏗️ [PhotosTab] build CALLED. Folders loaded: $_foldersLoaded, Count: ${_folders.length}");
                      ...
                      noItemsFoundIndicatorBuilder: (context) => Center(
                        child: Column(
                          ...
                          children: [
                            ...
                            Icon(Icons.photo_library_outlined, size: 64, color: colorScheme.onSurface.withOpacity(0.3)),
                            const SizedBox(height: 16),
                            Text(
                              _selectedFolder != null
                                  ? "No images in ${_formatFolderName(_selectedFolder!)}."
                                  : "No images yet.",
